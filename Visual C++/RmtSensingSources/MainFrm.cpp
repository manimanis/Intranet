...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndToolBar.CreateEx(this, TBSTYLE_FLAT | TBSTYLE_WRAPABLE,
		WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_FLOAT_MULTI
	...
	m_wndToolBar.SetWindowText("Fichier");
...
void CMainFrame::OnEditPaste() 
{
	...
	if (OpenClipboard())
	{
		...
		destFile = fDlg.GetPathName();

		if (destFile.Right(4) != ".bmp" && destFile.Right(4) != ".BMP") destFile += ".bmp";

		bmpManip.SaveBitmap(&dc, &bmp, destFile);
		bmp.Detach();
		...
		while(curTemplatePos != NULL)
		{
			...
			if(str == _T("RmtSen"))
			{
				curTemplate->OpenDocumentFile(destFile);
				return;
			}
		}	
