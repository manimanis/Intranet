...
{
	if (DrawingRect.Width() <= 1000 || DrawingRect.Height() >= -2000) return;
	...
	{
		LogFont.lfEscapement = 0; // 0 degree
		LogFont.lfHeight = 1300;
		LogFont.lfWidth = HistogrammeRect.Width() / (HistoTitle.GetLength() * 2 + 1);
...
HistoPointData Histogramme::DPtoLP(CPoint point)
{
	...
	hpData.x = float(float(point.x - DrawingRect.left) / float(DrawingRect.Width()) * 
				(Xmax - Xmin) + Xmin);
	hpData.y = float(Ymax - float(DrawingRect.bottom - point.y) / float(DrawingRect.Height()) * 
				(Ymax - Ymin));
