...
void CPictureHistogrammeView::OnInitialUpdate()
{
	...
	if (graphType == PICTURE_HISTOGRAMME) pDoc->SetDefaultMax();

	toolTip.Create(this);
	toolTip.SetDelayTime(20);
	toolTip.SetDelayTime(TTDT_AUTOPOP, 6000);
...
void CPictureHistogrammeView::OnDraw(CDC* pDC)
{
	...
	pDoc->max = (graphType == PICTURE_HISTOGRAMME) ? pDoc->max : nbrPix;
	...
	Histogramme_Image.SetYmin((double)pDoc->max);
	Histogramme_Image.SetYmax(0);

	Histogramme_Image.SetXscl(sclX);
	Histogramme_Image.SetYscl((double) pDoc->max / max_div_scl);
	...
	toolTipRect = Histogramme_Image.GetDrawingRect();
	pDC->LPtoDP(&toolTipRect);

	if (toolTipAssigned)	toolTip.DelTool(this, 4000);
		else				toolTipAssigned = TRUE;

	toolTip.AddTool(this, LPSTR_TEXTCALLBACK, &toolTipRect, 4000);
...
void CPictureHistogrammeView::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	...
	popup.LoadMenu(IDR_ZOOM);
	popup.GetSubMenu(2)->TrackPopupMenu(TPM_LEFTALIGN, point.x, point.y, this);
...
void CPictureHistogrammeView::OnOutilsParamtres() 
{
	...
	picHistDlg.m_YMaxPixelsDbl = pDoc->max;
	...
	if (picHistDlg.m_YMaxType == 2) pDoc->max = (int)picHistDlg.m_YMaxPixelsDbl;
	else if (picHistDlg.m_YMaxType == 1) pDoc->max = (int) (nbrPix * picHistDlg.m_YMaxPercentDbl / 100);
	else pDoc->SetDefaultMax();
	...
	case 1:
		max_div_scl = pDoc->max / (nbrPix * picHistDlg.m_maxY_percent_value / 100); break;
	case 2:
		if (graphType == PICTURE_HISTOGRAMME)
			max_div_scl = pDoc->max / picHistDlg.m_maxY_pixels_value;
		else
...
void CPictureHistogrammeView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (Histogramme_Image.IsInDrawingRect(point))
	{
		if (ComposanteRouge ||ComposanteBleu || ComposanteVerte)
		{
			...
			hpData.x = (float)floor(hpData.x);

			if (graphType == PICTURE_HISTOGRAMME)
			{
				rvbValues[0] = pDoc->picData[512 + int(hpData.x) % 256]; // r
				rvbValues[1] = pDoc->picData[256 + int(hpData.x) % 256]; // g
				rvbValues[2] = pDoc->picData[int(hpData.x) % 256]; // b
			}
			else
			{
				rvbValues[0] = pDoc->cumData[512 + int(hpData.x) % 256];
				rvbValues[1] = pDoc->cumData[256 + int(hpData.x) % 256];
				rvbValues[2] = pDoc->cumData[int(hpData.x) % 256];
			}

			str.Format("f(l : %d) = (r : %d, v : %d, b : %d)", int(hpData.x), rvbValues[0],
								rvbValues[1], rvbValues[2]);

			toolTip.UpdateTipText(LPCSTR(str), this, 4000);
			toolTip.Update();
		}
	}
}

BOOL CPictureHistogrammeView::OnToolTip(UINT id, NMHDR *pTTTStruct, LRESULT *pResult)
{
	TOOLTIPTEXT* pTTT = (TOOLTIPTEXT*)pTTTStruct;

	strcpy(pTTT->szText, "text");

	return TRUE;
}

BOOL CPictureHistogrammeView::PreTranslateMessage(MSG* pMsg) 
{
